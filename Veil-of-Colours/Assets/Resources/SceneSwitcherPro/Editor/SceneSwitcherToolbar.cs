...
public static class SceneSwitcherToolbar
{
    ...
    static void RefreshSceneList()
    {
        ...
        else
        {
            ...
            var validScenes = EditorBuildSettings
                .scenes.Where(scene => scene.enabled && File.Exists(scene.path))
                .Select(scene => Path.GetFileNameWithoutExtension(scene.path))
                .ToArray();

            // Detect deleted scenes still listed in Build Settings
            var missingScenes = EditorBuildSettings
                .scenes.Where(s => s.enabled && !File.Exists(s.path))
    ...
    {
        string currentScene = Path.GetFileNameWithoutExtension(
    ...
    static void UpdateSceneSelection()
    {
        ...
        if (currentScene != lastActiveScene)
        {
            ...
            sceneNames = sceneNames
                .Where(name => !name.EndsWith(" (not in build index)"))
                .ToArray();
    ...
    static void LoadScene(string sceneName)
    {
        ...
        {
            var buildScene = EditorBuildSettings.scenes.FirstOrDefault(scene =>
                scene.enabled && Path.GetFileNameWithoutExtension(scene.path) == sceneName
            );
